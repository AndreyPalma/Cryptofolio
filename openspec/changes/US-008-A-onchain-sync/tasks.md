...
- [ ] **T01** — Create `db/migrations/0003_wallet_last_synced_block.sql`
...
- [ ] **T02** — Run migration against Supabase (manual step)
...
- [ ] **T03** — Verify `.env.example` already has `ETHERSCAN_API_KEY` and `BSCTRACE_API_KEY`
...
- [ ] **T04** — Add `ApiKeyMissingError` and `ExternalApiError` to `apps/backend/src/services/errors.ts`
...
- [ ] **T05** — Create `apps/backend/src/sync/clients/on-chain-api.ts` (after T04)
...
- [ ] **T06** — Create `apps/backend/src/sync/classify.ts` — types only (after T05)
...
- [ ] **T07** — Create `apps/backend/src/sync/cost-resolver.ts` — `CostResolution` type only (after T05)
...
- [ ] **T08** — Create `apps/backend/src/schemas/sync.ts` (after T06, T07)
...
- [ ] **T09** — Create `apps/backend/src/sync/constants/routers.ts`
...
- [ ] **T10** — Write failing test: EtherscanClient `fetchNormalTransactions` returns typed `NormalizedTx[]` (engine)
...
- [ ] **T11** — Write failing test: EtherscanClient pagination triggers re-fetch when `result.length === 1000` (engine)
...
- [ ] **T12** — Implement `apps/backend/src/sync/clients/etherscan.ts` (after T10, T11)
...
- [ ] **T13** — Write failing test: BSCTraceClient `nr_getAssetTransfers` response normalized to `NormalizedTx` shape (engine)
...
- [ ] **T14** — Implement `apps/backend/src/sync/clients/bsctrace.ts` (after T13)
...
- [ ] **T15** — Write failing test: `groupByTxHash` groups normal + token txs by hash (engine)
...
- [ ] **T16** — Write failing test: `classifyAndDecomposeTransaction` — ETH native transfer → 1 TRANSFER_IN row (engine)
  - File: `apps/backend/src/sync/__tests__/classify-decompose.test.ts`
...
- [ ] **T17** — Write failing test: `classifyAndDecomposeTransaction` — token transfer to contract → 1 BUY row (engine)
  - File: `apps/backend/src/sync/__tests__/classify-decompose.test.ts`
...
- [ ] **T18** — Write failing test: `classifyAndDecomposeTransaction` — SWAP via router → 2 rows SWAP_OUT + SWAP_IN (engine)
  - File: `apps/backend/src/sync/__tests__/classify-decompose.test.ts`
...
- [ ] **T19** — Write failing test: `classifyAndDecomposeTransaction` — token-to-token swap → 2 rows (engine)
  - File: `apps/backend/src/sync/__tests__/classify-decompose.test.ts`
...
- [ ] **T20** — Write failing test: `classifyAndDecomposeTransaction` — `isError='1'` → returns [] (engine)
  - File: `apps/backend/src/sync/__tests__/classify-decompose.test.ts`
  - Group: normalTx with `isError='1'`
  - Assert: returns empty array

- [ ] **T21** — Implement `apps/backend/src/sync/classify.ts` — `groupByTxHash` and `classifyAndDecomposeTransaction` (after T15–T20)
...
- [ ] **T22** — Write failing test: `resolveTransferCost` — step 1: on-chain wallet with OPEN position → INHERITED (sync)
...
- [ ] **T23** — Write failing test: `resolveTransferCost` — step 2: Binance TRANSFER_OUT with same tx_hash → INHERITED from CEX (sync)
  - File: `apps/backend/tests/sync-transfer-cost.test.ts`
...
- [ ] **T24** — Write failing test: `resolveTransferCost` — step 1 CLOSED position falls through to MANUAL (sync)
  - File: `apps/backend/tests/sync-transfer-cost.test.ts`
  - Seed: wallet WC on-chain, position CLOSED for T1; no Binance tx with txHash=Y
  - Assert: returns `{ costSource: 'MANUAL', priceUsd: null }`

- [ ] **T25** — Write failing test: `resolveTransferCost` — step D: no match in any step → MANUAL (sync)
  - File: `apps/backend/tests/sync-transfer-cost.test.ts`
  - No wallet with fromAddress, no Binance tx with txHash
  - Assert: returns `{ costSource: 'MANUAL', priceUsd: null }`

- [ ] **T26** — Implement `apps/backend/src/sync/cost-resolver.ts` — `resolveTransferCost` function (after T22–T25)
...
- [ ] **T27** — Write failing test: `fetchAllRawTransactions` — pagination: mock returns 1000 then 50 → total 1050 (engine)
  - File: `apps/backend/src/sync/__tests__/pagination.test.ts`
...
- [ ] **T28** — Implement `apps/backend/src/services/on-chain-sync.ts` — `OnChainSyncService` (after T26, T27, T21)
...
- [ ] **T29** — Write failing e2e test: `POST /api/sync/:walletId` — 400 for CEX wallet (e2e)
...
- [ ] **T30** — Write failing e2e test: `POST /api/sync/:walletId` — 400 for missing ETHERSCAN_API_KEY (e2e)
  - File: `apps/backend/tests/e2e-sync.test.ts`
...
- [ ] **T31** — Write failing e2e test: `POST /api/sync/:walletId` — 404 for wallet not belonging to user (e2e)
  - File: `apps/backend/tests/e2e-sync.test.ts`
  - Seed wallet belonging to user U2; auth'd as U1
  - Assert: 404; variant: walletId is valid UUID but doesn't exist → 404

- [ ] **T32** — Write failing e2e test: `POST /api/sync/:walletId` — 400 for non-UUID walletId (e2e)
  - File: `apps/backend/tests/e2e-sync.test.ts`
  - `POST /api/sync/not-a-uuid` with valid JWT
  - Assert: 400 Zod validation error (never reaches handler)

- [ ] **T33** — Write failing e2e test: `POST /api/sync/:walletId` — 401 for missing JWT (e2e)
  - File: `apps/backend/tests/e2e-sync.test.ts`
  - No Authorization header; assert: 401 from global middleware (handler not reached)

- [ ] **T34** — Write failing e2e test: `POST /api/sync/:walletId` — 502 on Etherscan HTTP failure (e2e)
  - File: `apps/backend/tests/e2e-sync.test.ts`
...
- [ ] **T35** — Write failing e2e test: `POST /api/sync/:walletId` — 200 success + idempotency (e2e)
  - File: `apps/backend/tests/e2e-sync.test.ts`
...
- [ ] **T36** — Implement `apps/backend/src/routes/sync.ts` — Fastify plugin `syncRoutes` (after T28–T35)
...
- [ ] **T37** — Register `syncRoutes` in `apps/backend/src/index.ts` (after T36)
...
- [ ] **T38** — Run all vitest projects and confirm they pass (after T37)
...
- [ ] **T39** — Run typecheck and confirm zero errors (after T37)
  - `npm run typecheck`
  - Fix any type errors introduced across the new files

- [ ] **T40** — Commit: `feat(sync): add on-chain sync service for ETH and BSC wallets` (after T38, T39)
