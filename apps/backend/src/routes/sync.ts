...
import { createAlchemyClient } from "../sync/clients/alchemy.js";
...
export const syncRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  function createOnChainService() {
    const alchemyEth = createAlchemyClient({
      apiKey: process.env.ALCHEMY_API_KEY ?? "",
      network: "ETH",
      log: fastify.log,
    });
    const alchemyBsc = createAlchemyClient({
      apiKey: process.env.ALCHEMY_API_KEY ?? "",
      network: "BSC",
      log: fastify.log,
    });
  ...
  fastify
    ...
    .get("/:walletId/stream", { schema: { params: SyncParamsSchema } }, async (req, reply) => {
